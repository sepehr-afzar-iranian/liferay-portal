...
@Component(
        ...
        immediate = true,
        service = DeleteAuditMessagesListener.class
)
public class DeleteAuditMessagesListener extends BaseMessageListener {
    ...
    protected void activate(Map<String, Object> properties) {
        ...
        String cronExpression = _auditMessageAutoDeleterConfiguration.CronExpression();
    ...
    protected void doReceive(Message message) throws Exception {
        ...
        Timestamp thresholdTimestamp = new Timestamp(System.currentTimeMillis() - (month * 30L * 24L * 60L * 60L * 1000L));
        try {
            ActionableDynamicQuery actionableDynamicQuery = _auditEventLocalService.getActionableDynamicQuery();
            actionableDynamicQuery.setAddCriteriaMethod(dynamicQuery ->
                    dynamicQuery.add(RestrictionsFactoryUtil.lt("createDate", thresholdTimestamp))
            );
            actionableDynamicQuery.setPerformActionMethod((AuditEvent auditEvent) ->
                    _auditEventLocalService.deleteAuditEvent(auditEvent.getAuditEventId())
            );
            actionableDynamicQuery.performActions();
        } catch (Exception e) {
            _log.error("Error deleting old audit events", e);
        }
    }
